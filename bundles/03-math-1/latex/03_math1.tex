...
Mathematics is quite essential for  programmers that want to improve themselves in competitive programming. Topics introduced here are going to be used from graph theory to the subject of strings.  Therefore, a strong understanding of mathematics is fundamental.

\section{Number Theory}
Number theory is the study of natural numbers. Numbers are split into several groups. Number theory is related to the connection between these different groups of numbers  \cite{numbertheory}.
\begin{itemize}
  \item \textbf{Even Numbers} \hspace{19mm} 2, 4, 6, 8, 10, 12, ...
  \item \textbf{Cubic Numbers} \hspace{19mm} 1, 8, 27, 64, ...
  \item \textbf{Fibonacci Numbers} \hspace{10mm} 1, 1, 2, 3, 5, 8, 13, ...
\end{itemize}
...
The main question that comes to mind is how to check if a given number is prime or not. Let us see a function that returns if the given number is prime or not.  

We are going to test the number of n as  \textbf{1000000007} for each prime number algorithm.  It is a very large prime number. It allows us to see the run-time differences between the algorithms.
\clearpage

\subsubsection{Naive Approach: Linear Search}
